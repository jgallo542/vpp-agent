...
func (h *InterfaceVppHandler) dumpInterfaces(ifIdxs ...uint32) (map[uint32]*vppcalls.InterfaceDetails, error) {
	...
	for {
		...
		ifaceName := ifDetails.InterfaceName
...
func (h *InterfaceVppHandler) DumpMemifSocketDetails() (map[string]uint32, error) {
	...
	for {
		...
		filename := socketDetails.SocketFilename
	...
	for _, ifIdx := range ifIdxs {
		...
		for {
			...
			ifaceState := vppcalls.InterfaceState{
				SwIfIndex:    uint32(ifDetails.SwIfIndex),
				InternalName: ifDetails.InterfaceName,
...
func (h *InterfaceVppHandler) dumpMemifDetails(ifs map[uint32]*vppcalls.InterfaceDetails) error {
	...
	for {
		...
		_, ifIdxExists := ifs[uint32(memifDetails.SwIfIndex)]
		if !ifIdxExists {
			continue
		}
		ifs[uint32(memifDetails.SwIfIndex)].Interface.Link = &interfaces.Interface_Memif{
		...
		ifs[uint32(memifDetails.SwIfIndex)].Interface.Type = interfaces.Interface_MEMIF
	}
...
func memifModetoNB(mode memif.MemifMode) interfaces.MemifLink_MemifMode {
	switch mode {
	case 0:
