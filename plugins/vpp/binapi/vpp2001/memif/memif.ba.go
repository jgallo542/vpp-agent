...
type MemifCreate struct {
	...
	NoZeroCopy    bool
	HwAddr        MacAddress
	XXX_SecretLen uint32 `struc:"sizeof=Secret"`
	Secret        string `binapi:",limit=24"`
}
...
type MemifDetails struct {
	...
	BufferSize    uint16
	Flags         IfStatusFlags
	XXX_IfNameLen uint32 `struc:"sizeof=IfName"`
	IfName        string `binapi:",limit=64"`
}
...
type MemifSocketFilenameAddDel struct {
	IsAdd                 bool
	SocketID              uint32
	XXX_SocketFilenameLen uint32 `struc:"sizeof=SocketFilename"`
	SocketFilename        string `binapi:",limit=128"`
}
